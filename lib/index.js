...
function handle(i18next) {
  ...
  return function (req, res, next) {
    ...
    if (!req.lng && i18next.services.languageDetector) {
      ...
      var _i18next$services$languageDetector$detect = i18next.services.languageDetector.detect(req, res);

      var _i18next$services$languageDetector$detect2 = _slicedToArray(_i18next$services$languageDetector$detect, 2);

      lng = _i18next$services$languageDetector$detect2[0];
      detector = _i18next$services$languageDetector$detect2[1];
    } // set locale
    ...
    if (detector === 'path' && options.removeLngFromUrl) {
      var first = '';
      var pos = i18next.services.languageDetector.options.lookupFromPathIndex;
      if (req.url[0] === '/') {
        pos++;
        first = '/';
      }
